WITH all_sales AS (
 ...
 FROM (SELECT d_year
       ...
       FROM catalog_sales
       JOIN item ON i_item_sk=cs_item_sk AND i_category='Shoes'
       JOIN date_dim ON d_date_sk=cs_sold_date_sk AND d_year IN (1999, 2000)
       LEFT JOIN catalog_returns ON (cs_order_number=cr_order_number AND cs_item_sk=cr_item_sk)
       UNION ALL  -- Use UNION ALL if you don't need to eliminate duplicates (faster)
       SELECT d_year
             ,i_brand_id
             ,i_class_id
             ,i_category_id
       ...
       FROM store_sales
       JOIN item ON i_item_sk=ss_item_sk AND i_category='Shoes'
       JOIN date_dim ON d_date_sk=ss_sold_date_sk AND d_year IN (1999, 2000)
       ...
       FROM web_sales
       JOIN item ON i_item_sk=ws_item_sk AND i_category='Shoes'
       JOIN date_dim ON d_date_sk=ws_sold_date_sk AND d_year IN (1999, 2000)
