WITH cs_ui AS (
  ...
  JOIN cs_ui
    ON ss.ss_item_sk = cs_ui.cs_item_sk
  JOIN date_dim AS d1
  ...
  WHERE
    ...
    AND i.i_color IN ('light', 'cyan', 'burnished', 'green', 'almond', 'smoke')
    AND i.i_current_price BETWEEN 22 AND 32
    AND i.i_current_price BETWEEN 23 AND 37
  GROUP BY
    i.i_product_name,
    i.i_item_sk,
    s.s_store_name,
